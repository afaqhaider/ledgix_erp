...
class WeatherService {
  ...
  Future<WeatherInfo> fetchWeather(String city, TemperatureUnit unit) async {
    ...
    print('WeatherService: URL: $_baseUrl?q=$encodedCity&appid=$maskedKey&units=$units');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('WeatherService: Error Body: ${response.body}');
        if (response.statusCode == 401) {
          throw Exception('Weather API Key is invalid or not yet active. (OpenWeatherMap 401)');
        } else if (response.statusCode == 404) {
          throw Exception('City "$city" not found. Please check spelling in company settings.');
        } else if (response.statusCode == 429) {
          throw Exception('Weather API rate limit exceeded. Please try again later.');
        } else {
          try {
      ...
      if (e is http.ClientException) {
        throw Exception('Network error or CORS issue while fetching weather data.');
      }
      rethrow;
    }
  }
