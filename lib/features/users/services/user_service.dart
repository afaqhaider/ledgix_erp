...
class CompanyUserService {
  ...
  CollectionReference _getUsersRef(String companyId) {
    return _firestore.collection('companies').doc(companyId).collection('users');
  }

  Stream<List<CompanyUserModel>> getCompanyUsers(String companyId) {
    return _getUsersRef(companyId).snapshots().map((snapshot) {
      return snapshot.docs.map((doc) {
        return CompanyUserModel.fromMap(doc.data() as Map<String, dynamic>, doc.id);
      }).toList();
    });
  }
  ...
  Future<void> inviteUser({
    ...
    final userRef = _getUsersRef(companyId).doc(); // Temporary ID or based on email hash
  ...
  Future<void> updateUserRole(String companyId, String userId, UserRole role) async {
    ...
    batch.update(_firestore.collection('users').doc(userId), {'role': role.name});
    
    await batch.commit();
  }

  Future<void> updateUserStatus(String companyId, String userId, UserStatus status) async {
    final batch = _firestore.batch();
    
    batch.update(_getUsersRef(companyId).doc(userId), {'status': status.name});
    batch.update(_firestore.collection('users').doc(userId), {'status': status.name});
