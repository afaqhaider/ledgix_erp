  ...
  Future<void> _submitForApproval() async {
    ...
    try {
      await _approvalService.submitForApproval(
        ...
        sourceId: _currentBill.id,
        sourceNumber: _currentBill.billNumber,
        amount: _currentBill.totalAmount,
      );

      setState(() {
        _currentBill = _currentBill.copyWith(approvalStatus: 'pending');
      });
      ...
      if (mounted) {
        showErpError(
          ...
          message: 'Could not submit bill for approval.',
          error: e,
        );
      }
    } finally {
  ...
  Future<void> _postToAccounting() async {
    ...
    try {
      ...
      if (updatedDoc.exists && mounted) {
        setState(() {
          _currentBill = BillModel.fromMap(
            updatedDoc.data()!,
            updatedDoc.id,
          );
        });
      ...
      appBar: AppBar(
        ...
        actions: [
          if (!bill.isPosted) ...[
            ...
            else if (bill.approvalStatus == null ||
          ...
          if (bill.approvalStatus != null)
            Padding(
              ...
              child: Chip(
                ...
                backgroundColor: bill.approvalStatus == 'approved'
                    ? Colors.green
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (bill.reference != null && bill.reference!.isNotEmpty)
                              _buildInfoRow(context, 'Ref #', bill.reference!),
                          ],
                        ),
                      ),
                    ],
                    ...
                    child: SizedBox(
                      ...
                      child: Column(
                        children: [
                          ...
                          _buildTotalRow('VAT Amount', bill.vatAmount),
  ...
  Widget _buildApprovalHistory() {
    return StreamBuilder<List<ApprovalRequestModel>>(
      stream: FirebaseFirestore.instance
          ...
          .where('sourceId', isEqualTo: _currentBill.id)
          .snapshots()
          .map(
            (snap) => snap.docs
