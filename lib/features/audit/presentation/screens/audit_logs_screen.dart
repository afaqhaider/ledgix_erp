...
class _AuditLogsScreenState extends State<AuditLogsScreen> {
  ...
  final List<String> _modules = [
    'customers', 'suppliers', 'invoices', 'quotations', 'payments', 
    'journalEntries', 'chartOfAccounts', 'approvals', 'settings'
  ];

  final List<String> _actions = [
    'create', 'edit', 'delete', 'approve', 'reject', 'post', 'login', 'logout'
  ];
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.user.role.name != 'Admin' && widget.user.role.name != 'Owner') {
      return const Center(child: Text('Access Denied: Only Admins/Owners can view audit logs.'));
    }
                ...
                Expanded(
                  child: DropdownButtonFormField<String>(
                    ...
                    items: [
                      ...
                      ..._modules.map((m) => DropdownMenuItem(value: m, child: Text(m.toUpperCase()))),
                    ],
                ...
                Expanded(
                  child: DropdownButtonFormField<String>(
                    ...
                    items: [
                      ...
                      ..._actions.map((a) => DropdownMenuItem(value: a, child: Text(a.toUpperCase()))),
                    ],
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      leading: CircleAvatar(
                        ...
                        child: Icon(_getActionIcon(log.actionType), color: _getActionColor(log.actionType), size: 20),
                      ),
                      title: Text(log.description),
                      subtitle: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('By ${log.userName} in ${log.module.toUpperCase()}'),
                          if (log.documentNumber != null)
                            Text('Doc: ${log.documentNumber}', style: const TextStyle(fontWeight: FontWeight.bold)),
                          Text(
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              _detailRow('Time', DateFormat('yyyy-MM-dd HH:mm:ss').format(log.createdAt)),
              ...
              if (log.oldValues != null) ...[
                const Text('From:', style: TextStyle(color: Colors.red, fontSize: 12)),
                Text(log.oldValues.toString(), style: const TextStyle(fontSize: 11)),
                const SizedBox(height: 8),
              ],
              if (log.newValues != null) ...[
                const Text('To:', style: TextStyle(color: Colors.green, fontSize: 12)),
                Text(log.newValues.toString(), style: const TextStyle(fontSize: 11)),
              ],
            ],
          ),
        ),
  ...
  IconData _getActionIcon(String action) {
    switch (action) {
      ...
      case 'post': return Icons.account_balance;
