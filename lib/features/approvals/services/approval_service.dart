...
class ApprovalService {
  ...
  Stream<List<ApprovalRequestModel>> getMyRequests(String companyId, String userId) {
    return _getRef(companyId)
  ...
  Future<void> submitForApproval(ApprovalRequestModel request) async {
    ...
    await _auditService.log(
      ...
      description: 'Submitted ${request.sourceType} ${request.sourceNumber} for approval',
    );
  }
  ...
  Future<void> actionApproval({
    ...
    switch (sourceType) {
      case 'salesInvoice': collectionName = 'salesInvoices'; break;
      case 'quotation': collectionName = 'quotations'; break;
      case 'purchaseOrder': collectionName = 'purchaseOrders'; break;
      case 'supplierPayment': collectionName = 'supplierPayments'; break;
      case 'customerPayment': collectionName = 'customerPayments'; break;
      case 'journalEntry': collectionName = 'journalEntries'; break;
      default: throw 'Unknown source type: $sourceType';
    }

    batch.update(
      _firestore.collection('companies').doc(companyId).collection(collectionName).doc(sourceId),
    ...
    await _auditService.log(
      ...
      description: '${status == ApprovalStatus.approved ? 'Approved' : 'Rejected'} $sourceType $sourceNumber',
    );
  }

  Future<ApprovalRequestModel?> getRequestBySource(String companyId, String sourceId) async {
    ...
    return ApprovalRequestModel.fromMap(snapshot.docs.first.data() as Map<String, dynamic>, snapshot.docs.first.id);
  }
}
